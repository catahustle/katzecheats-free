...
float global_utils::random_float(float min, float max)
{
	static auto rf = reinterpret_cast<float(*)(float, float)>(GetProcAddress(GetModuleHandleA("vstdlib.dll"), "RandomFloat"));
	return rf(min, max);
}