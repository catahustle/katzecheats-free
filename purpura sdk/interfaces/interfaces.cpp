...
namespace interfaces {
	...
	void*						gameeventmanager = nullptr;
	void*						draw_screen_effect_material = nullptr;
	...
	void interfaces::init() {
		...
		localize = reinterpret_cast <i_localize*> (global_utils::get_interface<i_localize>((char*)"localize.dll", (char*)"Localize_001"));
		std::cout << "[interface:] localize: 0x" << localize << "\n";
		model_info = reinterpret_cast <iv_model_info*> (global_utils::get_interface<iv_model_info*>((char*)"engine.dll", (char*)"VModelInfoClient004"));
		std::cout << "[interface:] model_info: 0x" << model_info << "\n";
		gameeventmanager = global_utils::get_interface<void>((char*)"engine.dll", (char*)"GAMEEVENTSMANAGER002");
		std::cout << "[interface:] gameeventmanager: 0x" << gameeventmanager << "\n";
		...
		input = *reinterpret_cast<c_input**>(global_utils::get_signature("client_panorama.dll", "B9 ? ? ? ? F3 0F 11 04 24 FF 50 10") + 0x1);
		std::cout << "[interface:] input: 0x" << input << "\n";
		...
		using fn = uintptr_t(__thiscall*)(void*);
		fn* ptr = (*(fn**)engine_client);
		client_state = *reinterpret_cast<c_client_state***>((uintptr_t)ptr[12] + 0x10);
		std::cout << "[interface:] client_state: 0x" << client_state << "\n";
